...
class OutlineExtractor:
    ...
    def _parse_outline_response(self, response: str, chunk_index: int) -> List[Dict]:
        ...
        Returns:
            解析后的大纲结构
        """
        json_outlines = self._parse_json_outline_response(response, chunk_index)
        if json_outlines:
    ...
    def _parse_json_outline_response(self, response: str, chunk_index: int) -> List[Dict]:
        """兼容返回 JSON 数组的新提示词，并转换成后续步骤需要的 title/subtopics 结构。"""
        try:
        ...
        if isinstance(parsed, dict):
            for key in ("outline", "outlines", "topics", "items", "clips"):
        ...
        for index, item in enumerate(parsed, start=1):
            ...
            for field in ("summary", "reason", "product", "selling_point", "content", "cut_reason"):
                value = item.get(field)
                if isinstance(value, str) and value.strip():
                    subtopics.append(value.strip())
            ...
            for field in ("id", "start_time", "end_time", "product", "summary", "reason"):
                if item.get(field) is not None:
                    outline[field] = item[field]
