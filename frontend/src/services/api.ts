...
export const projectApi = {
    ...
    try {
      const project = await api.post<unknown, Project>('/projects/upload', formData, {
        ...
        onUploadProgress: (event) => {
          if (!data.onProgress || !event.total) return
          data.onProgress(Math.min(95, Math.round((event.loaded / event.total) * 95)))
        },
      })
      trackVideoImported({
        source: 'upload',
  ...
  downloadClipWithSubtitles: async (clipId: string): Promise<Blob> => {
    const response = await axios.get(`/api/v1/files/clips/${clipId}/download-subtitled`, {
      responseType: 'blob',
      headers: {
        'Accept': 'video/mp4'
      }
