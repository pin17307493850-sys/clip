...
const ClipCard: React.FC<ClipCardProps> = ({ 
  ...
  const [downloadingSubtitled, setDownloadingSubtitled] = useState(false)
  ...
  const handleDownloadWithSubtitles = async () => {
    setDownloadingSubtitled(true)
    try {
      const blob = await projectApi.downloadClipWithSubtitles(clip.id)
      const safeTitle = (clip.title || clip.generated_title || `clip_${clip.id}`)
        .replace(/[\\/:*?"<>|]/g, '')
        .slice(0, 80)
      const downloadUrl = window.URL.createObjectURL(new Blob([blob], { type: 'video/mp4' }))
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `${safeTitle}_带字幕.mp4`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      window.URL.revokeObjectURL(downloadUrl)
      message.success('带字幕视频已开始下载')
    } catch (error) {
      console.error('下载带字幕视频失败:', error)
      message.error('下载带字幕视频失败')
    } finally {
      setDownloadingSubtitled(false)
    }
  }
    ...
    <>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            key="download-subtitled"
            icon={<DownloadOutlined />}
            loading={downloadingSubtitled}
