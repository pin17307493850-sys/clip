...
export const UnifiedStatusBar: React.FC<UnifiedStatusBarProps> = ({
  ...
  const ProgressRow = ({ label, percent, detail }: { label: string; percent: number; detail?: string }) => (
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginTop: 9, minWidth: 0 }}>
        <div style={{ minWidth: 0, flex: '1 1 auto', lineHeight: 1.25 }}>
          <div
            style={{
              color: progress?.is_stalled ? '#ff7875' : 'var(--ac-sub)',
              fontSize: 12.5,
              overflow: 'hidden',
            ...
            title={detail || label}
          >
            {progress?.is_stalled ? `${label}卡住` : label}
          </div>
          {detail && detail !== label ? (
            <div
              style={{
                color: progress?.is_stalled ? 'rgba(255,120,117,.82)' : 'var(--ac-muted)',
                fontSize: 11,
                marginTop: 2,
                overflow: 'hidden',
                textOverflow: 'ellipsis',
        ...
        {progress?.is_stalled && onRestart ? (
          <Button
            ...
            style={{ height: 22, padding: '0 8px', color: '#ff7875', border: '1px solid rgba(255,120,117,.35)', borderRadius: 4, flex: '0 0 auto' }}
          >
            重启
          </Button>
        ) : null}
        <span className="ac-mono" style={{ color: 'var(--ac-accent)', fontSize: 12.5, flex: '0 0 auto', minWidth: 34, textAlign: 'right' }}>{Math.round(percent)}%</span>
      </div>
    </div>
  )
  // 终态：小圆点 + 标签
