...
const SettingsPage: React.FC = () => {
  ...
  const providerConfig = {
    ...
    '302ai': {
      ...
      icon: <RobotOutlined />,
      color: '#1677ff',
      description: '302.AI OpenAI compatible API',
  ...
  const handleSave = async (values: any) => {
    try {
      ...
      const normalizeModelName = (modelName: unknown) => {
        if (Array.isArray(modelName)) {
          return modelName.filter(Boolean).at(-1) || 'qwen-plus'
        }
        return typeof modelName === 'string' && modelName.trim()
          ? modelName.trim()
          : 'qwen-plus'
      }
      const modelName = normalizeModelName(values.model_name)
