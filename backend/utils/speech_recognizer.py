...
class SpeechRecognizer:
    ...
    def generate_subtitle(
        ...
        try:
            if config.method == SpeechRecognitionMethod.WHISPER_LOCAL:
                return self._generate_subtitle_whisper_local(video_path, output_path, config, progress_callback)
            ...
            if (config.enable_fallback and 
                ...
                return self.generate_subtitle(video_path, output_path, fallback_config, progress_callback)
            else:
                raise
    ...
    def _generate_subtitle_whisper_local(
        self,
        video_path: Path,
        output_path: Path,
...
def generate_subtitle_for_video(
    ...
    return recognizer.generate_subtitle(video_path, output_path, config, progress_callback)
