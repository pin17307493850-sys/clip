...
def process_video_pipeline(
    ...
    try:
        ...
        try:
            ...
            pipeline_adapter = create_simple_pipeline_adapter(str(project_id), task_record_id)
            ...
            if result.get("status") == "failed":
                ...
                task_to_update = db.query(Task).filter(Task.id == task_record_id).first()
                if task_to_update:
                    task_to_update.status = TaskStatus.FAILED
                    task_to_update.error_message = error_msg
                    task_to_update.result_data = result
                else:
                    logger.warning("Task row disappeared before failure update: %s", task_record_id)
                ...
                if task_to_update:
                    ...
                    task_to_update.current_step = "处理完成"
                    task_to_update.result_data = result
                else:
                    logger.warning("Task row disappeared before success update: %s", task_record_id)
