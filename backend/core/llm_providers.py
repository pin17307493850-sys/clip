...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(self, api_key: str, model_name: str = "gpt-3.5-turbo", **kwargs):
        ...
        try:
            ...
            client_kwargs["timeout"] = kwargs.get("timeout", 60)
            client_kwargs["max_retries"] = kwargs.get("max_retries", 0)
...
class AI302Provider(OpenAIProvider):
    """302.AI OpenAI-compatible provider."""

    def __init__(self, api_key: str, model_name: str = "gpt-4o-mini", **kwargs):
        kwargs.setdefault("base_url", "https://api.302.ai/v1")
        super().__init__(api_key, model_name, **kwargs)

    def get_available_models(self) -> List[ModelInfo]:
        """Return common model names available through 302.AI."""
        return [
            ModelInfo(
                name="gpt-4o-mini",
                display_name="GPT-4o Mini",
                provider=ProviderType.AI302,
                max_tokens=128000,
                description="302.AI OpenAI-compatible gateway"
            ),
            ModelInfo(
                name="gpt-4o",
                display_name="GPT-4o",
